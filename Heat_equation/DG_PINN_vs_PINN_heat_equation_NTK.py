...
for seeds_num in seeds_nums:
    ...
    np.random.seed(seeds_num)
    beta = np.random.rand()
    X, T, U, X_train, X_validation, X_test, X_true = get_data(lambda_1_true, batch_sizes)
    epoch_loss_r = []
    epoch_loss_i = []
    epoch_loss_b = []
    epoch_loss_d = []
    ...
    savemat(f'../Results/dgpinn_heat_NTK_seed_{seeds_num}.mat',
    ...
    for epoch in range(iter_2):
        ...
        # Stopping condition
        if u_error < epsilon_2:
            print(f'Stopping early at epoch {epoch} as relative l2 error fell below 0.001')
            break
    ...
    savemat(f'../Results/pinn_heat_NTK_seed_{seeds_num}.mat',
            {'u_pred': u_pred, 'u_test': u_test, 'U_pred': U_pred.reshape(201,201), 'u_true': U, 'loss_r': epoch_loss_r, 
              'loss_d': epoch_loss_d, 'beta': epoch_beta, 'lambda1': epoch_lambda1, 'lambda2': epoch_lambda2, 'time': t22 - t11})