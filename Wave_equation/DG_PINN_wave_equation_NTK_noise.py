...
for snr in SNR:
    torch.manual_seed(seeds_num)
    u_noisy, X, T, U, X_train, X_test, X_true = get_data(lambda_1_true, batch_sizes, snr)
    epoch_loss_r = []
    epoch_loss_i1 = []
    epoch_loss_i2 = []
    epoch_loss_b = []
    ...
    savemat(f'dgpinn_wave_NTK_SNR_{snr}.mat',
            {'u_pred': u_pred, 'u_test': u_test, 'U_pred': U_pred.reshape(201,201), 'u_noise': u_noisy.reshape(201,201),'u_true': U, 
             'loss_r': epoch_loss_r, 'loss_i2': epoch_loss_i2, 'loss_b': epoch_loss_b,'loss_d': epoch_loss_d, 'alpha': epoch_alpha, 'lambda_r': epoch_lambda_r, 
