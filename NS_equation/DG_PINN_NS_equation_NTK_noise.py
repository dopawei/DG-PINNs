...
def add_noise2(signal1, signal2, signal3, snr_db):
    # Calculate signal power and convert SNR from dB
    signal_power = (np.mean(signal1 ** 2) + np.mean(signal2 ** 2) + np.mean(signal3 ** 2))/3
    snr_linear = 10 ** (snr_db / 10)

    # Calculate noise power and generate noise
    noise_power = signal_power / snr_linear
    np.random.seed(seeds_num)
    noise = np.random.normal(0, np.sqrt(noise_power), signal1.shape)

    # Add noise to the signal
    return noise

# Load training data
def get_data(batch_sizes, snr_db):
    ...
    noise = add_noise2(data_domain[:, 3:4], data_domain[:, 4:5], data_domain[:, 5:6], snr_db)

    u_noise = data_domain[:, 3:4] + noise
    v_noise = data_domain[:, 4:5] + noise
    p_noise = data_domain[:, 5:6] + noise
    ...
    def to_tensor_noise(ids):
        return {
            'x': torch.from_numpy(data_domain[ids, 0:1]).float().to(device),
            'y': torch.from_numpy(data_domain[ids, 1:2]).float().to(device),
            't': torch.from_numpy(data_domain[ids, 2:3]).float().to(device),
            'u': torch.from_numpy(u_noise[ids, :]).float().to(device),
            'v': torch.from_numpy(v_noise[ids, :]).float().to(device),
            'p': torch.from_numpy(p_noise[ids, :]).float().to(device),
        }

    X_train = {'PDE': to_tensor_noise(id_pde), 'data': to_tensor_noise(id_data)}
    ...
    X_validation = to_tensor_noise(id_validation)
    X_test = to_tensor(mask)
    X_true = to_tensor(ids_total)

    return u_noise, v_noise, p_noise, X_train, X_validation, X_test, X_true
...
for snr in SNR:
    torch.manual_seed(seeds_num)
    u_noise, v_noise, p_noise, X_train, X_validation, X_test, X_true = get_data(batch_sizes, snr)
    epoch_loss_r1 = []
    epoch_loss_r2 = []
    epoch_loss_r3 = []
    epoch_loss_du = []
    ...
    #train_pinn(model, optimizer, X_train, iters=iter_1, stopping_loss=stopping_loss)
    ...
    x_true = X_true['x'].cpu().detach().numpy()
    y_true = X_true['y'].cpu().detach().numpy()
    t_true = X_true['t'].cpu().detach().numpy()
    ...
    # Save data and model
    savemat(f'dgpinn_NS_NTK_SNR_{snr}.mat',
            {'u_noise':u_noise, 'v_noise': v_noise, 'p_noise': p_noise, 'x_true': x_true, 'y_true': y_true, 't_true': t_true,  
